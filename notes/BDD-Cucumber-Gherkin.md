...
/*
    ...
    scenario and the step in that scenario, Cucumber will provide it at run time. Note that which annotation you use is programmatically irrelevant: the only benefit of matching the annotation with the step type is to make it clearer to the reader what step is being handled by the method. The @And annotation could be used for all three of these methods, and Cucumber would still match them correctly. This is 
...
```
#### Pattern Matching: Cucumber Expressions vs. Regex
To link a Gherkin step to a Java method, Cucumber uses **pattern matching**. There are two primary ways to define these patterns:

1.  **Cucumber Expressions (Modern/Recommended):** As seen in the examples above, these use intuitive placeholders like `{string}`, `{int}`, or `{word}`. They are highly readable and designed to be the standard for most automation tasks.
2.  **Regular Expressions (Regex):** For more complex scenarios where Cucumber Expressions fall short (such as matching specific patterns of text or complex optionality), Cucumber supports standard Regex. 
    *   *Example:* `@When("^the user enters (\\d+) items$")` would match "the user enters 5 items".

**How the Mapping Works:**
When Cucumber runs a scenario, it performs a string comparison. It looks at the text of the Gherkin step and searches your "glue" code for a method whose annotation pattern matches that text. 

*   **The Keyword is Metadata:** The annotation (`@Given`, `@When`, `@Then`) serves as a label for the human reader and a way to organize code. However, the **text inside the quotes** is what actually performs the matching. 
*   **Parameter Injection:** If the pattern contains a placeholder (like `{string}`), Cucumber extracts the value from the Gherkin text and passes it as an argument to the method. This is known as **parameter injection**. By parameterizing your steps you can reuse them, cutting down on the total number of steps you need to implement. 

**Cucumber** has 11 different types it supports by default (taken from Cucumber documentation):
...
### Executing Tests
To execute Cucumber tests with Gradle, you have several options. If you are using a Cucumber/Gherkin plugin in your IDE, you can "execute" your feature files directly. Alternatively, you can create a test runner and facilitate test execution through the runner class. Another method is to use the Gradle "test" task to trigger your tests from the command line, which also allows for specifying individual tests to execute.
...
### Rule
Added in version 6 of **Cucumber**, the **Rule** keyword is used to group **Scenarios** that validate a specific business requirement or constraint. 

While a **Feature** describes a high-level capability (e.g., "User Registration"), a **Rule** allows you to break that capability down into the specific logic that governs it (e.g., "Usernames must be unique"). This prevents a **Feature** file from becoming a massive, unorganized list of **Scenarios** and provides a clearer structure for stakeholders to see exactly which business requirements are being tested.

(Keep your existing code example below this)
...
### Example
In **Cucumber** the **Scenario** and **Example** keywords are synonymous: they are interchangeable in your **feature files**. Similar to the steps they organize, they provide varied syntactical understanding to the programmer, but **Cucumber** views them as the same: an indicator of a new test. The Example keyword fits well with the Rule keyword to help provide lexical clarity on what is actually being tested.
```gherkin
Feature: User Registration
    Users can register with the coffee house to track their purchase history and accumulate points
...
**Cucumber** and Junit work well together: Junit provides many useful integrations with other 3rd party software, and **Cucumber** can facilitate testing that uses your **feature files** and step implementations. Depending on the version of Junit you are using you either need the `cucumber-junit` dependency (Junit4) or `cucumber-junit-platform-engine` dependnecy (Junit5). This example will use Junit 5 moving forward
```groovy
dependencies {
...
@SelectPackages({"selenium.steps", "features"})
@ConfigurationParameter(key = GLUE_PROPERTY_NAME, value = "selenium.steps")
