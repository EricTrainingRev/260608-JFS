...
### Headless Mode
In many professional environments, particularly in Continuous Integration (CI) pipelines like Jenkins or GitHub Actions, there is no physical monitor attached to the server. To run tests in these environments, you can use **Headless Mode**. A "headless" browser runs the browser engine in the background without rendering a visible User Interface (UI).

**Benefits of Headless Mode:**
* **Resource Efficiency:** Uses significantly less CPU and RAM because it doesn't have to render the actual pixels of the window.
* **Speed:** Tests often execute slightly faster because the overhead of drawing the UI is removed.
* **CI/CD Compatibility:** Allows tests to run on Linux servers or Docker containers that lack a graphical display environment (X11/GUI).

To enable headless mode, you add the `--headless` argument to your **Options** object:
```java
ChromeOptions options = new ChromeOptions();

// Enables headless mode (runs without a visible window)
options.addArguments("--headless");

// Common addition for Linux/Docker environments to prevent crashes
options.addArguments("--no-sandbox"); // Chrome specific, tells the browser not to run in its normal sandbox
options.addArguments("--disable-dev-shm-usage"); // Docker specific, allows for more memory usage
...
### Navigate Methods
**WebDrivers** provide a variety of methods for navigating web pages and retrieving metadata. The **get(url)** method is the most common way to navigate; it loads a new web page into the current browser window. While it is often used for initial navigation, it also populates the browser's history stack, allowing for subsequent "back" and "forward" actions.

For more granular control over the browser's history, Selenium provides the **navigate()** interface. While **navigate().to(url)** performs essentially the same function as **get(url)**, the `navigate()` interface is required to access history-specific commands:
* **navigate().back()**: Simulates clicking the browser's "Back" button.
* **navigate().forward()**: Simulates clicking the browser's "Forward" button.
* **navigate().refresh()**: Simulates clicking the browser's "Refresh" button.

To gather information about the current state of the browser, use **getCurrentURL()**, which returns the current web page's URL as a String, and **getTitle()**, which retrieves the title of the current web page as a String.
