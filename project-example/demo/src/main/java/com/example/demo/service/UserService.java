...
public class UserService {
  ...
  */
  ...
  // NOTE: the controller handles putting together the response, so if this method executes without
  // any
  //       exceptions being thrown the controller will assume success and return the appropriate
  // response
  public void registerUser(User user) {
    ...
    // if we have reached this point then we know the new credentials are valid and we can register
    // the
    // user
    userRepo.save(user);
  }
  ...
  */
  ...
  public boolean isUnique(String credential) {
    ...
    // NOTE: we want isPresent to return false, because that means the username is actually unique,
    // hence the "not" operator
    return !userOptional.isPresent();
  }

  /**
   * Authenticates a user by verifying their username and password. Returns the full User entity on
   * success so the controller can generate a JWT.
   */
