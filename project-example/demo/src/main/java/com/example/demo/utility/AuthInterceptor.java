...
/**
 * Intercepts incoming requests and validates the JWT from the Authorization header. If the token is
 * missing or invalid the request is rejected with a 401 Unauthorized. If valid, the user's ID and
 * username are attached to the request as attributes so controllers can access them without
