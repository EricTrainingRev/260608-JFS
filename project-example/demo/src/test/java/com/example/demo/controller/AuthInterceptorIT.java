...
/**
 * Integration tests for the AuthInterceptor — the Spring interceptor that validates JWT tokens on
 * protected endpoints (like /books).
 *
 * <p>These tests verify that the interceptor correctly rejects requests with: - Expired tokens -
 * Malformed Authorization headers (wrong prefix, missing entirely) - Tokens signed with a different
 * secret key ...and allows requests with valid tokens through.
 *
 * <p>We manually craft JWTs here (using JJWT) so we can control expiration, signing keys, etc.
 * without depending on the app's login flow for invalid-token scenarios.
 */
class AuthInterceptorIT extends BaseIntegrationTest {
  ...
  /**
   * Verifies that a token whose expiration date is in the past gets rejected. The interceptor
   * should respond with 401 and an "Invalid or expired token" message.
   */
  @Test
  ...
  /**
   * Verifies that an Authorization header that doesn't start with "Bearer " is rejected. The
   * interceptor checks the header format before even trying to parse the token.
   */
  @Test
  ...
  /**
   * Verifies that a request with NO Authorization header at all is rejected. Protected endpoints
  ...
  /**
   * Verifies that a token signed with a DIFFERENT secret key is rejected. Even though the token
   * structure is valid, the signature won't match what the server expects, so it should fail
   * verification.
   */
  @Test
  void request_withWrongSecretToken_returns401() {
    ...
    String wrongSecretToken =
        Jwts.builder()
            ...
            .expiration(
                new Date(System.currentTimeMillis() + 86400000)) // expires in 24h (not expired)
  ...
  /**
   * Verifies that a legitimately obtained token (via register + login) passes through the
   * interceptor successfully. We just confirm we DON'T get a 401.
   */
  @Test
