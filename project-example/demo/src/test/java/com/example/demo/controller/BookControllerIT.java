 ...
 *
 * <p>Each test registers a UNIQUE user so book collections don't interfere across tests. The tests
 * verify: - Creating books with valid/invalid data - Reading books (all, by ID, across users) -
 * Updating books with valid/invalid data - Deleting books - Proper 401 responses when auth is
 * missing
 *
 * <p>Extends BaseIntegrationTest for REST Assured config and the obtainAuthToken() helper.
 */
class BookControllerIT extends BaseIntegrationTest {
  ...
  /**
   * Happy path: creating a book with all valid fields returns 201 and the response body contains
   * the saved book with a generated ID.
   */
  @Test
  ...
  /**
   * Attempting to create a book WITHOUT an Authorization header should be blocked by the
   * AuthInterceptor before the controller is even reached.
   */
  @Test
  ...
  /**
   * Validation: a book with a blank title should be rejected with 400. The service layer enforces
   * that title is non-empty.
   */
  @Test
  ...
  /**
   * Validation: pageCount must be between 1 and 25000. Tests both below-minimum (0) and
   * above-maximum (25001) values.
   */
  @Test
  ...
  /**
   * After creating two books for a user, GET /books should return exactly those two books in a JSON
   * array, each with all expected fields populated.
   */
  @Test
  ...
  /**
   * GET /books/{id} with a valid book ID should return that specific book with all its fields
   * matching what was originally created.
   */
  @Test
  ...
  /**
   * GET /books/{id} with a UUID that doesn't correspond to any book should return 404 Not Found.
   */
  @Test
  ...
  /**
   * Users should only be able to see their OWN books. If User A creates a book, User B requesting
   * that book by ID should get 404 (not 403) to avoid leaking the existence of other users' books.
   */
  @Test
  ...
  /**
   * GET /books/{id} where {id} is not a valid UUID format should return 400. The controller
   * validates the path variable format before querying the DB.
   */
  @Test
  ...
  void getBooks_withoutAuth_returns401() {
    given().contentType(ContentType.JSON).when().get("/books").then().statusCode(401);
  }

  // ==================== Book Update Tests (Requirements 8.1–8.5) ====================

  /**
   * Happy path: PUT /books/{id} with valid updated data should return 200 and the response body
   * should reflect ALL the new values.
   */
  @Test
  ...
  /**
   * PUT /books/{id} where {id} doesn't exist should return 404. You can't update a book that was
   * never created.
   */
  @Test
  ...
  /**
   * Validation on update: blank title and out-of-range pageCount should both return 400. Tests two
   * invalid scenarios against the same book.
   */
  @Test
  ...
  /**
   * PUT /books/{id} where {id} is not a valid UUID string should return 400 with a descriptive
   * error message, without hitting the database.
   */
  @Test
  ...
  /**
   * Happy path: DELETE /books/{id} for an existing book returns 204 No Content. 204 means "success,
   * but there's nothing to send back in the body."
   */
  @Test
  ...
  /**
   * After deleting a book, attempting to GET it by ID should return 404. This confirms the delete
   * actually removed the record from the database.
   */
  @Test
  ...
  /**
   * DELETE /books/{id} where {id} doesn't exist should return 404. Can't delete something that
   * isn't there.
   */
  @Test
  ...
  void deleteBook_withoutAuth_returns401() {
    given().when().delete("/books/" + UUID.randomUUID()).then().statusCode(401);
  }

  /**
   * DELETE /books/{id} where {id} is not a valid UUID format should return 400 with a descriptive
   * message.
   */
  @Test
