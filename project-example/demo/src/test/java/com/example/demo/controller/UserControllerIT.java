 ...
 *
 * <p>These tests verify: - Successful registration with valid credentials - Registration validation
 * (duplicate username, null fields, password rules, length) - Successful login returning a JWT -
 * Login failure scenarios (wrong password, non-existent user, null fields)
 *
 * <p>Each test uses unique usernames to avoid cross-test interference. Extends BaseIntegrationTest
 * for REST Assured config and helpers.
 */
class UserControllerIT extends BaseIntegrationTest {
  ...
  /**
   * Happy path: valid username (5-15 chars) + valid password (has uppercase, lowercase, and digit)
   * should return 201 with an empty body.
   */
  @Test
  void register_withValidCredentials_returns201() {
    given()
        .log()
        .all() // tells REST Assured to log all the request data
        ...
        .then()
        .log()
        .all() // tells REST Assured to log all of the response data
        .statusCode(201) // 201 Created = account registered
        .body(emptyOrNullString()); // No body content on successful registration
  }

  /**
   * Usernames must be unique. Registering the same username twice should return 400 with an error
  ...
  /**
   * A null username should be caught by validation and return 400. Note: We use HashMap here
   * because Map.of() doesn't allow null values.
   */
  @Test
  ...
  /**
   * A null password should be caught by validation and return 400. Same HashMap trick since
   ...
   * "abcdefg" has only lowercase — should be rejected.
   */
  @Test
  ...
  /**
   * Both username and password must be between 5 and 15 characters. "ab" is only 2 chars — below
   * the minimum length of 5.
   */
  @Test
  ...
  /**
   * Username exceeding 15 characters should also be rejected. "thisUsernameIsWayTooLong" is well
   * over the 15-char limit.
   */
  @Test
  ...
  /**
   * Happy path: after registering, logging in with correct credentials should return 200 with a JWT
   * in the response body. A valid JWT has exactly 3 Base64URL-encoded segments separated by dots.
   */
  @Test
  ...
  /**
   * Logging in with the wrong password (user exists but password doesn't match) should return 401
   * Unauthorized with a generic error message. We use a generic message to avoid revealing whether
   * the username exists.
   */
  @Test
  ...
  /**
   * Logging in with a username that was never registered should return 401. Same generic error
   * message as wrong-password to prevent username enumeration.
   */
  @Test
  ...
  /**
   * Sending null username and password to login should return 401. The service layer checks for
   * null fields before attempting authentication.
   */
  @Test
